#!/usr/bin/env python3
"""ghost-check style heuristics: reference implementation of web/ghost-check.js.

The web app scores text with these heuristics (see README, "Web app"). The CLI
(ghost_check.py) uses the trained classifier instead. Kept in Python so the web
engine can be tested (tests/check_parity.py) and calibrated (eval/).
